# This Makefile is for the ZHOUYI extension to perl.
#
...
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/ZHOUYI.pm]
#     clean => { FILES=>q[ZHOUYI-*] }
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/ZHOUYI.pm $(INST_MAN3DIR)/ZHOUYI.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''The ZHOUYI is module interpreter and outer the Chinese ancient philosophy of The Book of Change(易经);'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: ZHOUYI' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "The ZHOUYI is module interpreter and outer the Chinese ancient philosophy of The Book of Change(易经);",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "ZHOUYI",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="ZHOUYI" VERSION="0.11">' > ZHOUYI.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>The ZHOUYI is module interpreter and outer the Chinese ancient philosophy of The Book of Change(易经);</ABSTRACT>' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>orange &lt;linzhe@ijz.me&gt;</AUTHOR>' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-5.18" />' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> ZHOUYI.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> ZHOUYI.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/ZHOUYI.pm' 'blib/lib/ZHOUYI.pm' 
